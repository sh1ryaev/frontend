   ...
   <div>
    ...
    <slide v-for="(slide,index) in item.photos" :key="index">
      <div class="carousel__item">
        <MDBCardImg @click="showLightBox" :src="slide" alt="..." style="z-index: -1;" />
      </div>
    </slide>
...
<div v-else>
  <a @click="showLightBox"><img class="rounded d-block w-100" v-bind:src="item.photos[0]" alt="..." style="width: 100%;  object-fit: cover;">
    <transition name="fade" mode="out-in">
   ...
   <transition name="fade" mode="out-in">
     <a v-if="active & item_id===item.id & type!='mypost' & item.photos.length>1"  href="/profile"
              ...
              <img class=" m-1" v-bind:src="'https://shiryaev2001django.pythonanywhere.com'+item.user_photo" alt="..."
   ...
   <transition name="fade" mode="out-in" style="width: 100%">
      ...
      <MDBCardTitle
                    style="position: absolute; left: 3%;top: 3%; font-size: 14px;color: white">
        <a class=" btn btn-light btn-sm text-muted"  role="button" @click="changeCat(item.cat_id)" >
...
export default {
  ...
  methods:{
    ...
    showLightBox(){
                axios.get('/posts-photo/'+this.item_id+'/').then(response => {
    ...
    getCategories(){
        ...
        get_Posts() {
          ...
          axios.get('/posts?cat_id='+str+'&search='+search+'&page='+page).then(response => {
            ...
            let posts = response.data;
              axios.get('/user-like/'+localStorage.getItem('user_id')+'/').then(async response => {
                ...
                for (var i = 0; i < posts.length; i++) {
                  ...
                  if(this.$store.state.isAuth)
                  {
                    is_liked = ans.indexOf(posts[i].id)
                  }
    ...
    savePost()
    {
      axios.put('posts/'+this.item_id+'/',{title:this.title,description:this.description,category_id:this.ch_category,id_user:localStorage.getItem('user_id')})
      .then(response=>{
        console.log(response)
